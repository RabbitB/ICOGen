...
static func class_inherits_from(a_class, ancestor) -> bool:
	var class_proxy: ClassInfoProxy = ClassInfoProxy.new(a_class)
	var ancestor_proxy: ClassInfoProxy = ClassInfoProxy.new(ancestor)
	return class_proxy.inherits_from(ancestor_proxy)


static func class_has_signal(for_class, signal_name: String) -> bool:
	var proxy: ClassInfoProxy = ClassInfoProxy.new(for_class)
	return proxy.has_signal(signal_name)


static func class_get_signal(from_class, signal_name: String) -> Dictionary:
	var proxy: ClassInfoProxy = ClassInfoProxy.new(from_class)
	return proxy.get_signal(signal_name)


class ClassInfoProxy extends Reference:
	...
	func inherits_from(ancestor: ClassInfoProxy) -> bool:
		match _type:
			BUILT_IN:
				return ancestor.get_type() == BUILT_IN && \
						ClassDB.is_parent_class(_class, ancestor.get_class())
			SCRIPT_DB:
