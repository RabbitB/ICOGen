...
static func error(
			error_msg: String,
			msg_args: Array = [],
			log_call_stack: bool = false) -> void:
...
static func _call_stack_to_string(call_stack: Array) -> String:
	...
	for call in call_stack:
		...
		var call_string: String = "function: %s, line: %d, source: %s" % \
				[call["function"], call["line"], call["source"]]
...
static func _message(
			...
			call_discard_depth: int = 2) -> String:
	...
	if log_call_stack:
		...
		#	It's necessary to remove a number of calls (declared by
		#	call_discard_depth) from the call-stack, so that we return an
		#	accurate representation of where this message was actually logged
		#	from. The end-user doesn't care that the actual top of the stack is
		#	this utility function; they care about logging where they called it
		#	from. As this is a private function of the Log class, it's largely
		#	called as a helper by other Log class functions, and thus the
		#	default discard depth of the call-stack is 2. One for this function,
		#	and one for the other Log function that called it.
